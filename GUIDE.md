...
| Current step | **B.2.3** (preparation `I1`: tooling ticket, config and fixed file, coverage audit, RAR-S75 registration; then the maintainer runs the pilot and the N = 10,000 tune), then **B.2.4** (the `[0,10]` gate) |
...
- [ ] **B.2** Cluster 1 — selectivity core: TT eval storage, correction, histories, picker, move-loop pruning, LMR — **READY_FOR_IMPLEMENTATION / I2**
    ...
    - [ ] **B.2.3** SPSA on the `b2core` arm: 82 `CoreParams` coordinates (switches, `CoreIirMinDepth` and `CoreEvalRule50Damping` out), `config_b23core.json`, the `-Tune -Features b2core` tooling ticket, pilot 128 × 32, N = 10,000 registered as RAR-S75, final theta baked, fitted-vs-unfitted diagnostic run — **READY_FOR_IMPLEMENTATION / V**
