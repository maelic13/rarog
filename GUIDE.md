...
| Current step | **B.2.7** (re-tune on Colosseum, RAR-S78): registered and handed over 2026-09-22, the 150,000-game tune and its `[0,3]` gate are the maintainer's to run; it closes B.2; **B.3.1** (cluster 2 implementation, `I2`) was **returned to RESEARCH a fourth time on 2026-09-23, after amendment 4's bounds**. The bounds are built and IIR is shrunk to its default, but they do not bound seldepth: the default arm reaches the ply cap at depth 17, at up to 300× the head's nodes, and the head itself reaches it at depth 19 (`analysis/b3_research_2026-09-23.md`, Unknown 10). CI and the close-out wait for the packet's amendment; the leaf's games still come after the tune's sessions. B.2.6 closed 2026-09-22. **E.3.1** (tag-driven release flow, `I1`, added 2026-09-22) is independent tooling that may land at any point before E.3 |
...
|---|---|---|
| B.3.1 returned to RESEARCH after amendment 4's bounds (`90a7f87`), 2026-09-23: seldepth is not bounded by them (arm at the ply cap from depth 17, up to 300× the head's nodes; the head at the cap from depth 19) | A research amendment defines the bound by cost at depths 16–20 and picks one that closes the arm's excess (packet *Amendment 4 record*; the implementation's view: a per-line extension budget); then step 2, CI and close-out | B.3.2 |
