...
- [ ] **B.2** Cluster 1 — selectivity core: TT eval storage, correction, histories, picker, move-loop pruning, LMR; both gates passed and the fitted core is the default build, open only for the re-tune B.2.7 — **GAME_GATE / I2**
    ...
    - [ ] **B.2.7** Re-tune on Colosseum of the coordinates at least one step from their seeds at N = 5,000, from the final theta; SPRT `[0,3]` vs the B.2.4b head; fully registered 2026-09-22: 17 coordinates at RAR-S75's steps, N = 5,000 × 30 games, `r_end` 0.0031, `rarog-b27core-tune.exe` at 7,185,678, gate cap 40,000 pairs, dry run resolved; **the tune is the maintainer's to run** (about 31–33 h); weather-factory stays installed until the gate resolves (RAR-S78) — **IMPLEMENTED / V**
