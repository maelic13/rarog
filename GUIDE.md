...
| Current step | **B.2.6.1**, which waits only on its prepared short live run on the released runner; then **B.2.7** (re-tune), which closes B.2; **B.3** (cluster 2 — NMP, ProbCut, singular and extensions, `I2`) after that. B.2.6.2 closed 2026-09-21 and B.2.6.3 on 2026-09-22 |
...
|---|---|---|
| B.2.6.1's one short live run, re-prepared on the released runner 2026-09-22 | The 200-game `colosseum.ps1 -Mode match` run completes on an idle host and its run directory is read | B.2.6.1 ticks; B.2.6 closes; B.2.7 launches |
