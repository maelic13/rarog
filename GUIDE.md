...
| Current step | **B.3** (cluster 2 — NMP, ProbCut, singular and extensions, `I2`) on the accepted head; **B.2.6** (Colosseum) when `cli-v0.1.0` is tagged, **B.2.7** (re-tune) after it |
...
- [ ] **B.2** Cluster 1 — selectivity core: TT eval storage, correction, histories, picker, move-loop pruning, LMR — **READY_FOR_IMPLEMENTATION / I2**
    ...
    - [ ] **B.2.6** Adopt Colosseum CLI as the harness, once `cli-v0.1.0` is released and qualified in its own repository, B.2.3's tune has finished on weather-factory and B.2.4b is gated with `sprt.ps1`; fastchess stays staged as the second runner — **RESEARCH / M**
        - [ ] **B.2.6.1** Policy as committed run files and thin `sprt`/`spsa` wrappers that keep the sidecar, fingerprint, compiler-equality and dirty-tree guards; `setup_tools.ps1` stages the tagged release and pins its SHA-256; resolved-configuration parity against the `sprt.ps1` manifest and one short live run. Run files and the surface converter landed 2026-09-21 by maintainer request, ahead of the tag; wrappers, staging and parity remain — **RESEARCH / I1**
