...
| Current step | **B.2.7** (re-tune on Colosseum, RAR-S78): registered and handed over 2026-09-22, the 150,000-game tune and its `[0,3]` gate are the maintainer's to run; it closes B.2; **B.3.1** (cluster 2 implementation, `I2`) was **returned to RESEARCH a third time on 2026-09-23, after T4**: amendment 3 is applied (T3's gates pass, KBNK passes), but with IIR off or in the Stockfish form the arm's lines extend to the ply cap and `bench 13` does not finish (`analysis/b3_research_2026-09-23.md`, Unknown 9). CI and the close-out wait for the packet's amendment; the leaf's games still come after the tune's sessions. B.2.6 closed 2026-09-22. **E.3.1** (tag-driven release flow, `I1`, added 2026-09-22) is independent tooling that may land at any point before E.3 |
...
|---|---|---|
| B.3.1 returned to RESEARCH after T4 (`e90d8ba`), 2026-09-23: under `CoreIirPolicy` 0 or 2 singular extensions run lines to the ply cap (seldepth 127) and `bench 13` does not finish; the default arm's seldepth is up to twice the head's | A research amendment chooses among options (a)–(c) in the packet's *T4 record* (recommended: (c), SF's alpha return at an exclusion node with no other move plus a shrunk IIR categorical); then CI and close-out | B.3.2 |
