...
| Current step | **B.2.7** (re-tune on Colosseum, RAR-S78): registered and handed over 2026-09-22, the 150,000-game tune and its `[0,3]` gate are the maintainer's to run; it closes B.2; **B.3.1** (cluster 2 implementation, `I2`) was **returned to RESEARCH a second time on 2026-09-23, after T3**: P5's attempts, three-ply and multi-cut gates fail on live counters, each traced to a contract term, and the donor multi-cut fails the KBNK regression test on the arm (`analysis/b3_research_2026-09-23.md`, Unknown 8). T4 waits for the packet's amendment; the leaf's games still come after the tune's sessions. B.2.6 closed 2026-09-22. **E.3.1** (tag-driven release flow, `I1`, added 2026-09-22) is independent tooling that may land at any point before E.3 |
...
|---|---|---|
| B.3.1 returned to RESEARCH after T3 (`40dcd93`), 2026-09-23: P5's attempts (3.24%), three-ply (21%) and multi-cut (32%) gates fail, from the donor candidate floor, the non-PV two-ply bar and the donor multi-cut rule; that multi-cut rule also fails the KBNK regression test on the arm | A research amendment chooses among options (a)–(d) in the packet's *T3 record* (recommended: (c) plus (a), Rarog's multi-cut with the other legs as reads); T4 then resumes | B.3.2 |
...
- [ ] **B.3** Cluster 2 — NMP, ProbCut, singular/multi-cut/negative/LDSE extensions, IIR policy; researched on the fitted head 2026-09-23 (RAR-S79); SPRT `[0,3]`; back in research after B.3.1's T3 — **RESEARCH / I2**
    - [ ] **B.3.1** Implement behind `b3proof`: NMP at cut nodes with a verification region, ProbCut's donor depth and return, graded singular/multi-cut/LDSE and the LMR term, IIR switch; off arm exact. T1–T2 built 2026-09-23; two premises corrected by amendment 2 (ProbCut option (a), region kept); T3 built, then returned: three P5 activation gates fail on live counters (packet Unknown 8); T4 waits — **RESEARCH / I2**
