...
| Development head | `dev` at the flip (`a47e85b`), version **2.5.0-dev**; fingerprint **7,185,678 / EBF 2.444** — the B.2.3-fitted selectivity core, a default feature since B.2.4b accepted it (`14a7079` baked the theta; the unfitted core read 4,706,910 / EBF 2.391). `--no-default-features` still compiles the superseded B.1 search at **7,601,220 / EBF 2.474**, reporting `+legacy`, until B.8 deletes it. B.1, B.2.0 and B.2.0.2's MultiPV were behaviour-neutral at that fingerprint (RAR-P24, RAR-P25, RAR-P26), and B.2.2.1's clamp conversion (6,586,667) was reverted by `308abe9` after RAR-S74's run (g). Last verified on Windows ARM64 and macOS ARM64 at the 2.4.0 head (RAR-P19); pinned `rustc 1.98.1` |
...
| Pool position, `3+0.03` 1T, **2.5.0-dev** | Rating **3191** against 2.4.0's 3001 (fit at 3,900, native; 6,000-game gauntlet, pool held at its Super Rating Tournament ratings, RAR-M57, 2026-09-19): Houdini 4 −105, Houdini 3 −77, Critter 1.6a −27, Stockfish 5 −26, Houdini 1.5a +4, Shredder 13 +4, Fritz 16 +16, Rybka 4.1 +64, Rarog 2.4.0 +168, Basilisk 1.10.0 +186 |
...
| Current step | **B.2.7** (re-tune on Colosseum, RAR-S78): registered and handed over 2026-09-22, the 150,000-game tune and its `[0,3]` gate are the maintainer's to run; it closes B.2; **B.3.1** (cluster 2 implementation, `I2`) was **returned to RESEARCH a fifth time on 2026-09-23, after amendment 5's budget**. The per-line extension budget is built (arm `bench 13` at 7,479,114, +4.1% on the head), but the deep-iteration cost screen fails at two of eleven comparisons: 3.44× the head's nodes at position 11 depth 17, and 4.11× at position 15 depth 19 (`analysis/b3_research_2026-09-23.md`, Unknown 11). Counters, suites, CI and the close-out wait for the packet's amendment; the leaf's games still come after the tune's sessions. B.2.6 closed 2026-09-22. **E.3.1** (tag-driven release flow, `I1`, added 2026-09-22) is independent tooling that may land at any point before E.3 |
...
|---|---|---|
| B.3.1 returned to RESEARCH after amendment 5's budget (`9bf6717`), 2026-09-23: the deep-iteration cost screen fails (position 11 depth 17 at 3.44×, position 15 depth 19 at 4.11× the head's nodes); the arm finishes every depth the head does | A research amendment decides whether the pointwise two-position 3× rule is the right instrument, and whether any residual excess is a defect or a B.3.3 seed question (packet *Amendment 5 record*); then counters, suites, CI and close-out | B.3.2 |
...
- [ ] **B.2** Cluster 1 — selectivity core: TT eval storage, correction, histories, picker, move-loop pruning, LMR; both gates passed and the fitted core is the default build, open only for the re-tune B.2.7 — **GAME_GATE / I2**
    ...
    - [ ] **B.2.7** Re-tune on Colosseum of the coordinates at least one step from their seeds at N = 5,000, from the final theta; SPRT `[0,3]` vs the B.2.4b head; fully registered 2026-09-22 and amended the same day before any game: all 82 coordinates restarted at theta_5000 with RAR-S75's steps (`b27all`), N = 5,000 × 30 games, `r_end` 0.0031, `rarog-b27core-tune.exe` at 7,185,678, gate cap 40,000 pairs, dry run resolved; **the tune is the maintainer's to run** (about 31–33 h); weather-factory stays installed until the gate resolves (RAR-S78) — **IMPLEMENTED / V**
...
- [ ] **B.3** Cluster 2 — NMP, ProbCut, singular/multi-cut/negative/LDSE extensions, IIR policy; researched on the fitted head 2026-09-23 (RAR-S79); SPRT `[0,3]`; back in research after amendment 5's cost screen — **RESEARCH / I2**
    - [ ] **B.3.1** Implement behind `b3proof`: NMP at cut nodes with a verification region, ProbCut's donor depth and return, graded singular/multi-cut/LDSE and the LMR term, IIR switch; off arm exact. T1–T2 built 2026-09-23; two premises corrected by amendment 2 (ProbCut option (a), region kept); T3 built, then amendment 3 (Rarog's multi-cut rule and candidate floor, bars kept) applied; T4 built, then amendment 4 (the extension chain bounded: `alpha` for a lone excluded move, `ply < 2·root_depth`) built, `CoreIirPolicy` shrunk to its default; amendment 5's per-line extension budget built; returned: the deep-iteration cost screen fails at two depths (3.44×, 4.11× the head's nodes; packet Unknown 11); counters, CI and close-out wait — **RESEARCH / I2**
