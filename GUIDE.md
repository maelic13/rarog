...
- [ ] **B.3** Cluster 2 — NMP, ProbCut, singular/multi-cut/negative/LDSE extensions, IIR policy; researched on the fitted head 2026-09-23 (RAR-S79); SPRT `[0,3]`; B.3.1 implemented 2026-09-23, B.3.2 closed 2026-09-24, B.3.3 next — **IMPLEMENTED / I2**
    ...
    - [ ] **B.3.3** Bake `SingularTtDepthMargin=2` on the arm (one engine commit), curvature sweep, then the SPSA on Colosseum if curved (maintainer-run); theta baked; bake and sweep done 2026-09-24 (3 of 7 curved), RAR-S81 registered 2026-09-25 (blocks of 2,000 × 30 under PLAN rule 7c, up to three); configuration, tune binary and dry run done the same day; **block 1 is the maintainer's to run** — **IMPLEMENTED / V**
