...
- [~] 5.1 Search-constant SPSA wave (pruning, LMR, futility, ProbCut margin, TM); incl. relocated 2.11 Group-B widen `[0,120]` and 2.5.2 futility-direction A/B. **Prep DONE (2026-06-29):** ceilings widened; `ProbCutMargin`, `FutilityImprovingDir` (0/1 A/B), `LazyMargin`, and the 7-param TM group all exposed (tune-gated, ×10000 where float); configs + `setup_spsa.ps1` groups + SPSA README done. Bench prep no-op; 159/159 tests, fmt clean, options hidden in release.
  - **Pruning group SPSA DONE (2,461 iters / 78,752 games, tc=3+0.03), candidate baked, SPRT pending.** Tuned: `FutilityBase 86→61`, `FutilityNotImproving 49→42`, `RazoringCoeff→193`, `NullMoveDepthCoeff 15→10`, `NullMoveImprovingBonus 25→33`, `LmpBase 115→88`, `LmpNotImproving 57→63`, `QuietHistPruneCoeff 4419→5072`, `SeePruningCoeff→84`, `SeePruningMax→808`, `AspirationDelta→30`, `SingularBetaMult 4→6` (interior after widening 6→8), `LmpCountBase 2`. Candidate **bench `3,956,393`** (was `4,747,104`; ~17% more aggressive pruning). Next: user runs the `[0,3]` SPRT vs the `phase47` head — keep on H1.
