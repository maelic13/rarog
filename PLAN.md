...
| Next implementation step | **Phase 5 step 1, IN PROGRESS — pruning group SPSA done, awaiting its SPRT.** Prep (all wiring) complete: ceilings widened `[0,60]→[0,120]`; `ProbCutMargin` `[60,400]`, `FutilityImprovingDir` 0/1 A/B, `LazyMargin` `[200,2000]`, and the 7-param ×10000 TM group all exposed (tune-gated); configs + `setup_spsa.ps1` groups + SPSA README done. **Pruning SPSA (2026-06-29): tc=3+0.03, 2,461 iters / 78,752 games**, candidate baked into `SearchParams::default()` (`FutilityBase 86→61`, `FutilityNotImproving 49→42`, `RazoringCoeff 191→193`, `NullMoveDepthCoeff 15→10`, `NullMoveImprovingBonus 25→33`, `LmpBase 115→88`, `LmpNotImproving 57→63`, `QuietHistPruneCoeff 4419→5072`, `SeePruningCoeff 81→84`, `SeePruningMax 811→808`, `AspirationDelta 31→30`, `SingularBetaMult 4→6` [interior after widening ceiling 6→8], `LmpCountBase 2`). Candidate **bench `3,956,393`** (was `4,747,104` — ~17% fewer nodes, more aggressive pruning at the final eval scale). Tests 159/159, fmt clean. **Next action:** user runs the `[0,3]` SPRT of the pruning candidate vs the `phase47` head; keep on H1, revert on H0. Then the `lmr`/`futility`/`probcut`/`tm` groups, plus the `LazyMargin` widen+`[-3,3]` safety check and the `FutilityImprovingDir` A/B. |
