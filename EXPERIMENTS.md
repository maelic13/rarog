...
| RAR-S25 | Phase-4.3a provenance-hazard census at `d354d02`: can a consumer infer a producer from entry shape, given provenance is not persisted? Exact counters in the store path, `bench 13`, 1T. | **Observation.** A moveless store inherits the resident move, so **33,712 of 953,957 stand-pat stores (3.53%) walked away carrying a searched move**, becoming byte-identical to a searched qmove. A shape test of `depth 0 + Lower + has a move` — the only provenance-free way to grant searched qmoves capability while denying stand pat — therefore carries a **10.71% false-positive rate** (33,712 of 314,627). Separately, **62,821 horizon stores (3.50%)** overwrote a deeper same-position entry, which the depth-preservation rule only blocks beyond 3 plies. | Under this state 4.3 cannot cleanly separate stand pat from searched qmoves without persistence, which is the concrete trigger 4.2 registered for reopening the 1-bit provenance question (age 5→4 bits, `[0,3]` gate, RAR-S22). Suppressing the inheritance instead would make the inference exact but discards ordering evidence RAR-S24 measured as valuable, so it is not a free fix. A 3.5% hazard is small enough that the depth-0-versus-deeper split (which needs no move test and no persistence) remains sound — only the within-horizon split is blocked. | `src/tt.rs`; `tools/diag_search_quality.ps1`; `d354d02`; Plan 4.2–4.3 |
