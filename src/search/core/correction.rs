...
impl Searcher {
    ...
    fn correction_value(&self, board: &Board, ply: usize) -> i32 {
        ...
        let pawn = self.td.corr.correction_history[us]
            [infra::index(board.pawn_key()) & (CORR_SIZE - 1)] as i32;
        let minor = self.td.corr.minor_correction_history[us]
            [infra::index(board.minor_key()) & (CORR_SIZE - 1)] as i32;
        let own_non_pawn = self.td.corr.non_pawn_correction_history[us][us]
            [infra::index(board.non_pawn_key(color)) & (CORR_SIZE - 1)]
            as i32;
        let their_non_pawn = self.td.corr.non_pawn_correction_history[us][them]
            [infra::index(board.non_pawn_key(!color)) & (CORR_SIZE - 1)]
            as i32;
    ...
    pub(super) fn update_correction(&mut self, board: &Board, diff: i32, depth: i32, ply: usize) {
        ...
        if crate::diag::sampled(board.hash(), ply, crate::diag::SAMPLE_CORRECTION) {
            ...
            crate::diag::record_correction_slot(
                ...
                pawn_key,
                self.td.corr.correction_history[us][pawn_index],
            );
            crate::diag::record_correction_slot(
                1,
                us * CORR_SIZE + minor_index,
                minor_key,
                self.td.corr.minor_correction_history[us][minor_index],
            );
            crate::diag::record_correction_slot(
                2,
                us * 2 * CORR_SIZE + us * CORR_SIZE + own_index,
                own_key,
                self.td.corr.non_pawn_correction_history[us][us][own_index],
            );
            crate::diag::record_correction_slot(
                3,
                us * 2 * CORR_SIZE + them * CORR_SIZE + other_index,
                other_key,
                self.td.corr.non_pawn_correction_history[us][them][other_index],
            );
        }
        update_hist_entry(
            &mut self.td.corr.correction_history[us]
                [infra::index(board.pawn_key()) & (CORR_SIZE - 1)],
            scaled,
            HISTORY_MAX,
        );
        update_hist_entry(
            &mut self.td.corr.minor_correction_history[us]
                [infra::index(board.minor_key()) & (CORR_SIZE - 1)],
            scaled,
            HISTORY_MAX,
        );
        update_hist_entry(
            &mut self.td.corr.non_pawn_correction_history[us][us]
                [infra::index(board.non_pawn_key(color)) & (CORR_SIZE - 1)],
            scaled,
            HISTORY_MAX,
        );
        update_hist_entry(
            &mut self.td.corr.non_pawn_correction_history[us][them]
                [infra::index(board.non_pawn_key(!color)) & (CORR_SIZE - 1)],
            scaled,
            HISTORY_MAX,
        );
