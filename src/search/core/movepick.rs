...
impl Searcher {
    ...
    fn append_scored_moves(
        ...
        let counter = if !previous.is_null() {
            self.td.hist.countermove[previous.from_sq().index()][previous.to_sq().index()]
        } else {
            Move::NULL
        };
        ...
        let killer0 = self.td.hist.killers[ply][0];
        let killer1 = self.td.hist.killers[ply][1];
        ...
        for &mv in moves {
            ...
            let score = if mv == tt_move {
                ...
                see = board.see(mv);
                let hist = self.td.hist.cap_history[attacker as usize][mv.to_sq().index()]
